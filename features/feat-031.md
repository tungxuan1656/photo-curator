...
- Download transport slice: `ModelDownloadDelegate` now writes `URLSessionDataDelegate` response data directly to the resumable partial file through a 256 KiB buffer, throttles progress publication, and retains Range resume plus manifest validation. The earlier chunked relay reached 8.7 MB/s versus 1.1 MB/s for the previous per-byte stream on the same host; the direct-to-file path has no fresh speed benchmark yet. These are host/network measurements, not iPhone claims.
