...
- Final-review fix wave: restored S14 entry on grid/groups/removed (Continue to Save) + overview/showReview push guards.
- PR-review fix wave (P2/P3): `finishSave` deletes persisted session data (cancelSave → delete, Completion holds in-memory state); confirmed Start New supersedes like discard; empty resume releases ownership + deletes + goes Home; Completion warns on missing-only partials; ReviewModel carries persisted `assetUnavailable`-decision unavailable count (`max` with live progress); already-on-failed-route retry sets an inline `reviewLoadRetryFailed` note; Home card shows `ResumeSnapshot.stageDescription` (ux-flows §7 phases) with matching VoiceOver label; `saveAlbum` rejects whitespace-only names via `.failed(.assetsUnavailable)`; PhotoDetail resets `loadFailed`/`cgImage` synchronously on pager change; SelectionToggle has an explicit 44pt frame. Files: `App/AppModel.swift`, `App/AppModel+Save.swift`, `Features/Review/{ReviewModel,ReviewOverview,Completion,PhotoDetail}.swift`, `Features/Onboarding/HomeView.swift`, `SharedUI/SelectionToggle.swift`.
